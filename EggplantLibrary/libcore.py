...
class EggplantLibDynamicCore:
    ...
    def __init__(self, suite: str | list[str] = '', host='', port='', scripts_dir: str = ''):
        """
        ...
        *Each of import parameter is optional and may stay unset during library import.*
        ...
        params: dict[str, Any] = {'host': 'http://127.0.0.1', 'port': '5400',
        ...
        suite_param = params.get('suite', [])
        if isinstance(suite_param, str):
            suite_param = [suite_param]
        self.eggplant_suite: list[str] = suite_param
        this_dir = os.path.abspath(os.path.dirname(__file__))

        # if suite path still not set, use first ".suite" dir in the library folder
        if not self.eggplant_suite:
        ...
        # otherwise suite path is set, but make sure it's absolute
            ...
            for i, suite in enumerate(self.eggplant_suite):
                if not os.path.isabs(suite):
                    self.eggplant_suite[i] = os.path.abspath(
                        os.path.join(this_dir, suite))
        ...
        self.keywords_dir = [os.path.join(
            e, params['scripts_dir']) for e in self.eggplant_suite]
        ...
        self.static_keywords: dict[str, Any] = dict()
    ...
    def __fetch_eggplant_keywords(self):
        ...
        """
        eggplant_keywords = dict()
        for directory in self.keywords_dir:
            eggplant_suite_keywords = self.get_scripts_from_folder(directory)
            eggplant_keywords = {
                **eggplant_keywords, **eggplant_suite_keywords}
    ...
    def run_keyword(self, name, args):
        ...
        if _keyword:
            ...
            log.warn("Eggplant keywords: {}".format(self.eggplant_keywords))
            command = self.eggplant_keywords.get(name, None)
            if not command:
                raise KeywordError(
                    "Cannot run keyword '{}'. Keyword not found! Available keyword: {}".format(name, self.eggplant_keywords))
    ...
    def get_keyword_source(self, name):
        ...
        if static_keyword:
            ...
            if name in ['__init__', '__intro__']:
                ...
                result = self.eggplant_keywords.get(name, None) + ":1"

        if not result:
            log.warn("Source not found for keyword: {}".format(name))
